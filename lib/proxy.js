    ...
    https.request = function (options, callback) {
            if (!isProxyRequired(options.host)) {
                if (isProxyGinRequired(options.host)) {
                    ...
                    newOptions.port = httpProxy.port;
                    if (options.protocol === 'https:') {
                        console.error('----> HTTPS Request w/ GIN');
                ...
                if (options.protocol === 'https:') {
                    console.error('----> HTTPS Request without proxy');
                    console.error(options);
                ...
                //Change proxy URL if URL is on GIN
                httpsProxy = isProxyGinRequired(options.href) ? url.parse("http://proxy-gin.internal.fr:3128") : url.parse(process.env.https_proxy);
