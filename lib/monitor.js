...
Monitor.prototype.findChecksNeedingPoll = function(callback) {
  ...
  if(process.env.http_proxy_api)
  {
    var httpProxy = url.parse(process.env.http_proxy_api);
    this.addApiHttpOption('hostname', httpProxy.hostname);
    this.addApiHttpOption('port', httpProxy.port);
    this.addApiHttpOption('protocol', httpProxy.protocol);
  }
