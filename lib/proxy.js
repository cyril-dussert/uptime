...
if (process.env.http_proxy) {
  ...
  http.request = function(options, callback) {
    ...
    httpProxy = isProxyGinRequired(options.href) ? url.parse("http://proxy-gin.internal.fr:3128") : httpProxy;
    new Error(httpProxy.hostname);
...
var isProxyGinRequired = function(hostname) {
    ...
    new Error('Gin : '+ pattern.test(hostname));
    console.log('Gin : '+ pattern.test(hostname));
